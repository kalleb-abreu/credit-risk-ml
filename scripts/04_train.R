...
if (!is.null(sample_n)) {
  message("sample_size=", sample_n, " — training on subsamples only")
}
...
for (dataset in datasets) {
  ...
  for (classifier in classifiers) {
    for (resampling in resamplings) {
      ...
      dir.create(
        here("predictions", "calibration", dataset, category),
        showWarnings = FALSE, recursive = TRUE
      )
      write_parquet(
        result$pred_calibration,
        here(
          "predictions", "calibration", dataset, category,
          paste0(pred_key, ".parquet")
        )
      )

      dir.create(
        here("predictions", "test", dataset, category),
        showWarnings = FALSE, recursive = TRUE
      )
      write_parquet(
        result$pred_test,
        here(
          "predictions", "test", dataset, category,
          paste0(pred_key, ".parquet")
        )
      )
    }
