...
for (path in cal_files) {
  ...
  dir.create(here("models", "calibrators", dataset, category), showWarnings = FALSE, recursive = TRUE)
  saveRDS(cals$platt,    here("models", "calibrators", dataset, category, paste0(pred_key, "_platt.rds")))
  saveRDS(cals$isotonic, here("models", "calibrators", dataset, category, paste0(pred_key, "_isotonic.rds")))
}
